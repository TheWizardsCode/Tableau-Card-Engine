...
> **Upgrade economy (CG-0MT3IYSRL001VVUP):** upgrade cards follow the business-card action economy. A market click **moves the upgrade to hand for 1 daily action**; applying it the same day is a **free composite**, and applying an upgrade held from a previous day costs **1 action**. Dragging an upgrade straight onto a matching business is a same-turn buy-and-apply at the **+50% premium** (1 action) — identical pricing to business buy-and-place. The headless `buy-upgrade` action is the same-day composite equivalent (1 action, listed cost), and both the AI and Monte Carlo account for the action cost, so upgrades never provide a free second daily action. Card pools, tier gates and the persistence schema are unaffected.
