...
> **Upgrade economy (CG-0MT3IYSRL001VVUP):** upgrade cards follow the business-card action economy. A market click **moves the upgrade to hand for 1 weekly action**; applying it the same week is a **free composite**, and applying an upgrade held from a previous week costs **1 action**. Dragging an upgrade straight onto a matching business is a same-turn buy-and-apply at the **+50% premium** (1 action) — identical pricing to business buy-and-place. The headless `buy-upgrade` action is the same-week composite equivalent (1 action, listed cost), and both the AI and Monte Carlo account for the action cost, so upgrades never provide a free second weekly action. Card pools, tier gates and the persistence schema are unaffected.

> **Synergy reclassification (Health type):** The Clinic (`biz-clinic`) was originally assigned the Service synergy type alongside Laundromat and Barbershop. It was reclassified to **Health** as part of introducing the new Health synergy type to M2. The Health type is a non-profit community health axis represented by Clinic, Private Clinic, and Pharmacy — cards that generate reputation per turn instead of (or in addition to) coin income. This reclassification affects the following cards: Clinic (cost 1000, income 0, rep +20/turn, Health), Private Clinic (cost 800, income 200, Health), Pharmacy (cost 600, income 100, Health), and their upgrades Medical Center (rep +10/turn) and Private Medical Center (income +200). The Service synergy type retains Laundromat, Barbershop, and Day Spa (bridge).
