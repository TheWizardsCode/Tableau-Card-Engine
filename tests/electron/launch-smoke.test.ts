...
/**
 * Failed resource loads (URLs) captured for the whole session.
 *
 * Kept separately from console errors: Chromium's console message for a
 * failed load is just "Failed to load resource: net::ERR_FILE_NOT_FOUND"
 * with no URL, so console text alone cannot distinguish which resource
 * failed (or whether it is a known-benign missing thumbnail).
 */
...
describe('Electron launcher smoke test', () => {
  ...
  it('surfaces no fatal renderer errors during the session', () => {
    ...
    // The URL is only visible via requestfailed, not in the console text.
    const fatalRequests = failedRequests.filter(
      url => !/thumbnail\.png$/.test(url),
    );
    const fatal = sessionErrors.filter(e =>
      /failed to load|net::|not allowed to load|uncaught|is not defined/i.test(e),
    );
    expect(fatal).toEqual([]);
    expect(fatalRequests).toEqual([]);
  });
});
