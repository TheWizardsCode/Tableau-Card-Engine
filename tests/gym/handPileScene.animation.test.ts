...
describe('GymHandPileScene source migration', () => {
  it('scene source no longer imports dealCard directly', () => {
    const fs = require('fs');
    ...
    // Should still use animateAddCard (via HandView)
    expect(source).toContain('animateAddCard');
  ...
  it('HandView public API remains backwards compatible', () => {
    // These should all still work
    const fs = require('fs');
    const path = require('path');
    const source = fs.readFileSync(
      path.resolve(__dirname, '../../src/ui/HandView.ts'),
      'utf-8',
    );

    expect(source).toContain('addCard');
    expect(source).toContain('removeCard');
    expect(source).toContain('setCards');
    expect(source).toContain('animateAddCard');
    expect(source).toContain('getCardCenters');
  });
});
