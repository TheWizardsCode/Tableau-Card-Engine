...
describe('UNIFIED_TUTORIAL_STEPS', () => {
  it('defines exactly 12 steps', () => { expect(UNIFIED_TUTORIAL_STEPS.length).toBe(12); expect(UNIFIED_TUTORIAL_STEP_COUNT).toBe(12); });
  it('steps have sequential T1-T12 IDs', () => { for(let i=0;i<12;i++) expect(UNIFIED_TUTORIAL_STEPS[i].id).toBe(`T${i+1}`); });
  ...
  it('has correct distribution: 8 confirm + 4 action', () => { expect(UNIFIED_TUTORIAL_STEPS.filter(s=>s.gate==='confirm').length).toBe(8); expect(UNIFIED_TUTORIAL_STEPS.filter(s=>s.gate==='action').length).toBe(4); });
  ...
  it('T11 is confirm gate with investmentsRow highlight', () => { expect(findStep('T11').gate).toBe('confirm'); expect(findStep('T11').highlightZone).toBe('investmentsRow'); });
  ...
  it('T12 is confirm gate with completionModal highlight', () => { expect(findStep('T12').gate).toBe('confirm'); expect(findStep('T12').highlightZone).toBe('completionModal'); });
});
...
describe('advanceTutorialStep', () => {
  ...
  it('advances through all 12 steps to index 12', () => { let s=startTutorial(createTutorialControllerState()); for(let i=0;i<12;i++) s=advanceTutorialStep(s); expect(s.currentStepIndex).toBe(12); });
...
describe('getCurrentStep', () => {
  ...
  it('returns null when past end (index 12)', () => { let s=startTutorial(createTutorialControllerState()); for(let i=0;i<12;i++) s=advanceTutorialStep(s); expect(getCurrentStep(s)).toBeNull(); });
});
