...
describe('UNIFIED_TUTORIAL_STEPS', () => {
  ...
  it('defines exactly 14 steps', () => { expect(UNIFIED_TUTORIAL_STEPS.length).toBe(14); expect(UNIFIED_TUTORIAL_STEP_COUNT).toBe(14); });
  it('steps have sequential T1-T14 IDs', () => { for(let i=0;i<14;i++) expect(UNIFIED_TUTORIAL_STEPS[i].id).toBe(`T${i+1}`); });
  ...
  it('has correct distribution: 8 confirm + 6 action', () => { expect(UNIFIED_TUTORIAL_STEPS.filter(s=>s.gate==='confirm').length).toBe(8); expect(UNIFIED_TUTORIAL_STEPS.filter(s=>s.gate==='action').length).toBe(6); });
  ...
  it('T9 is action gate with streetGrid highlight (place from hand)', () => { const t=findStep('T9'); expect(t.gate).toBe('action'); expect(t.requiredAction).toBe('place-business'); expect(t.highlightZone).toBe('streetGrid'); });
  it('T10 is confirm gate with centerModal highlight', () => { expect(findStep('T10').gate).toBe('confirm'); expect(findStep('T10').highlightZone).toBe('centerModal'); });
  ...
  it('T12 is confirm gate with challengePanel highlight', () => { expect(findStep('T12').gate).toBe('confirm'); expect(findStep('T12').highlightZone).toBe('challengePanel'); });
  it('T13 is confirm gate with hud highlight (score)', () => { expect(findStep('T13').gate).toBe('confirm'); expect(findStep('T13').highlightZone).toBe('hud'); });
  ...
  it('T8 is action gate with select-business requiredAction and requiredCardId (buy Bookshop + auto-place)', () => { const t=findStep('T8'); expect(t.gate).toBe('action'); expect(t.requiredAction).toBe('select-business'); expect(t.requiredCardId).toBe('biz-bookshop-0'); expect(t.highlightZone).toBe('marketBusinessRow'); });
  it('T14 is confirm gate with completionModal highlight', () => { expect(findStep('T14').gate).toBe('confirm'); expect(findStep('T14').highlightZone).toBe('completionModal'); });
});
...
describe('advanceTutorialStep', () => {
  ...
  it('advances through all 14 steps to index 14', () => { let s=startTutorial(createTutorialControllerState()); for(let i=0;i<14;i++) s=advanceTutorialStep(s); expect(s.currentStepIndex).toBe(14); });
...
describe('getCurrentStep', () => {
  ...
  it('returns null when past end (index 14)', () => { let s=startTutorial(createTutorialControllerState()); for(let i=0;i<14;i++) s=advanceTutorialStep(s); expect(getCurrentStep(s)).toBeNull(); });
});
